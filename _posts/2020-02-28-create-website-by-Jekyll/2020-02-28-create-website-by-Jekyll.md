...
如果不想要把直接使用系統中的Ruby，可以用[rbenv](https://github.com/rbenv/rbenv)建立一個local的環境，還可以指定要使用的ruby版本，詳細的安裝過程可以參考repository裡的README.md，底下是安裝的一個範例。

```bash
brew install rbenv
rbenv init
rbenv install 2.7.5
rbenv local 2.7.5  # Use ruby 2.7.5 for a local directory
```
...
如果在安裝的過程中出現eventmachine裝不起來，使用底下的指令也失敗

```bash
gem install eventmachine -v '1.2.7' --source 'https://rubygems.org/' --user
```

且看log的錯誤是出現在`#include "ruby/config.h"`，原因可能是因為Xcode把系統原生的Ruby環境弄髒了，可以參考[這邊的解法](https://github.com/CocoaPods/CocoaPods/issues/10286#issuecomment-750403566)看看。
